...
class FFmpegManager {
  ...
  constructor(options?: any) {
    ...
    this.ffmpeg.on('progress', this.onProgress.bind(this));
    watch(this.runTask, () => {
      this.startRun();
    });
  }
  ...
  mkdir(paths: string[]) {
    paths.forEach(async (filePath) => {
      await this.ffmpeg.createDir(filePath);
    });
  }
  // 进度
  private onProgress({ progress }: { progress: number }) {
  ...
  // 打印目录
  async logDir(filePath: string) {
    const log = await this.readDir(filePath);
  ...
  rmFile(filePath: string) {
    return this.ffmpeg.deleteDir(filePath);
  }
  // 判断文件是否存在
  async fileExist(filePath: string, fileName: string) {
    const data = await this.readDir(filePath);
    return data.map(node => node.name).indexOf(fileName) > -1;
  }
  // FS写文件
  async writeFile(filePath: string, fileName: string, fileUrl: string, force = false) {
    const isFileExist = await this.fileExist(filePath, fileName);
    if (force || !isFileExist) {
      await this.ffmpeg.writeFile(`${filePath}${fileName}`, await fetchFile(fileUrl));
    }
  ...
  async getFileUrl(filePath: string, fileName: string, format: string) {
    const fileBlob = await this.getFileBlob(filePath, fileName, format);
    return window.URL.createObjectURL(fileBlob);
  }
  /**
   * 获取文件file对象
   * */
  async getFile(filePath: string, fileName: string, format: string) {
    const localPath = `${fileName}.${format}`;
    const fileBuffer = await this.getFileBuffer(filePath, fileName, format);
  ...
  async mergeAudio(start: number, trackList: TrackItem[], trackAttrMap: Record<string, any>, fileName: string, filePath: string) {
    ...
    if (await this.fileExist(this.pathConfig.audioPath, fileName)) {
  ...
  async splitAudio(videoName: string, format: string, force = false) {
    ...
    const isFileExist = await this.fileExist(this.pathConfig.resourcePath, audioName);
    if (force || !isFileExist) {
  ...
  async genFrame(fileName: string, format: string, size: { w: number, h: number }) {
    ...
    const isResourceExist = await this.fileExist(this.pathConfig.resourcePath, filePath);
    if (!isResourceExist) {
      return { framePath };
    }
    const isFrameExist = await this.fileExist(this.pathConfig.framePath, fileName);
    if (isFrameExist) {
      return { framePath }
    };
  ...
  async genPlayFrame(videoName: string, format: string, size: { w: number, h: number }, time: number) {
    return new Promise(async (resolve) => {
      ...
      const isResourceExist = await this.fileExist(this.pathConfig.resourcePath, videoFilePath);
      if (!isResourceExist) {
        resolve({ framePath });
        return;
      }
      const isPlayFrameVideoExist = await this.fileExist(this.pathConfig.playFrame, videoName);
      if (!isPlayFrameVideoExist) {
        this.mkdir([framePath]);
      }
      const isPlayFrameFileExist = await this.fileExist(this.pathConfig.playFrame, fileName);
  ...
  async getPIFrame(videoName: string, format: string, size: { w: number, h: number }, start: number) {
    ...
    const framePath = `${this.pathConfig.playFrame}${videoName}`;
    const videoFilePath = `${videoName}.${format}`;
    const isVideoFilePathExist = await this.fileExist(this.pathConfig.resourcePath, videoFilePath);
    if (!isVideoFilePathExist) {
      return false;
    }
    const isVideoNameExist = await this.fileExist(this.pathConfig.playFrame, videoName);
    if (!isVideoNameExist) {
      this.mkdir([framePath]);
    }
    const { commands } = this.baseCommand.genPlayIFrame(`${this.pathConfig.resourcePath}${videoFilePath}`, framePath, size, start);
    await this.run(commands);
    const fileBuffer = await this.getFileBuffer(framePath, fileName, 'jpg');
  ...
  async getAudio(trackList: TrackItem[], trackAttrMap: Record<string, any>) {
    ...
    const isAudioExist = await this.fileExist(this.pathConfig.audioPath, fileName);
    if (!isAudioExist) {
      return {
        start, end,
        audioUrl: ''
    ...
    const audioUrl = await this.getFileUrl(this.pathConfig.audioPath, 'audio', 'mp3');
    return {
      start, end, audioUrl
    };
  }
